...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Ascend MiniMind LoRA Fine-tuning")
    ...
    parser.add_argument("--wandb_project", type=str, default="Ascend-MiniMind-LoRA")
    ...
    if args.use_wandb and is_main_process():
        ...
        wandb_run_name = f"Ascend-MiniMind-LoRA-{args.lora_name}-Epoch-{args.epochs}-BS-{args.batch_size}-LR-{args.learning_rate}"
    ...
    if args.use_compile == 1:
        args.use_compile = 0
        Logger('[LoRA] monkey-patch forward incompatible with torch.compile, disabled')
    if dist.is_initialized():
