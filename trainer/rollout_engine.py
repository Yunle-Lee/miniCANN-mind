"""
Rollout Engine for RL training.
NPU-adapted version with Torch and SGLang backends.
"""
import os
import sys
__package__ = "trainer"
...
from trainer.huawei_adapter import get_device
...
class TorchRolloutEngine(RolloutEngine):
    def __init__(self, policy_model, tokenizer, device=None, autocast_ctx=None):
        if device is None:
...
class SGLangRolloutEngine(RolloutEngine):
    def __init__(self, base_url, model_path, shared_ckpt_path="./sglang_ckpt", timeout=120):
    ...
    def rollout(self, prompt_ids, attention_mask, num_generations, max_new_tokens, temperature=0.8):
        ...
        if not isinstance(results, list):
            results = [results]
        all_output_ids, all_completion_ids, all_logprobs, completions = [], [], [], []
        for result in results:
            ...
            prompt = all_input_ids[len(all_output_ids)]
    ...
    def update_policy(self, model):
        ...
        if not dist.is_initialized() or dist.get_rank() == 0:
            try:
                ...
                print(f"[SGLANG WARNING] update_weights failed: {e}")
                ok = False
...
def create_rollout_engine(engine_type="torch", policy_model=None, tokenizer=None, device=None,
                          autocast_ctx=None, sglang_base_url=None, sglang_model_path=None,
                          sglang_shared_path=None):
